...
<%@ Import Namespace="TNQD.Common" %>
<%@ Import Namespace="TNQD.Database.Schema" %>
...
<script runat="server">
    ...
    {
        TaiKhoan tk = XacThuc.GetAccount();
        if (tk == null)
        {
            return false;
        }
        if (tk.IdGroup != (int)TNQD.Common.Enum.ConstantsEnum.GroupEnum.Admin)
        {
            return false;
        }
        return true;
